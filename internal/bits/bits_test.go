...
func TestBitstreamShift(t *testing.T) {
	...
	for i, tc := range []struct {
		...
		if got, want := StreamShift(tc.i), tc.o; !bytes.Equal(got, want) {
...
func TestFindPatterns(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		byteOffset, bitOffset := ScanStream(firstBlockMagicLookup, secondBlockMagicLookup, tc.buf)
	...
	for i := 6; i < 65; i++ {
		...
		for p := 0; p < (i-6)*8; p++ {
			...
			byteOffset, bitOffset := ScanStream(firstBlockMagicLookup, secondBlockMagicLookup, m)
...
func TestPartialFalsePositives(t *testing.T) {
	...
	for i, p := range partial {
		byteOffset, bitOffset := ScanStream(firstBlockMagicLookup, secondBlockMagicLookup, p)
		...
		for shift := 0; shift < 7; shift++ {
			...
			byteOffset, bitOffset := ScanStream(firstBlockMagicLookup, secondBlockMagicLookup, tmp)
